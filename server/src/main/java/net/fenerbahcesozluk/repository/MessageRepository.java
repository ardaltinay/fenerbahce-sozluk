...
public interface MessageRepository extends JpaRepository<Message, UUID> {
    ...
    @Query("SELECT m FROM Message m " + "WHERE ((m.sender = :user1 AND m.receiver = :user2) "
            + "    OR (m.sender = :user2 AND m.receiver = :user1)) " + "AND m.isDeleted = false "
    ...
    @Query("SELECT COUNT(m) FROM Message m " + "WHERE m.sender = :sender AND m.receiver = :receiver "
            + "AND m.readAt IS NULL " + "AND m.isDeleted = false")
     ...
     */
    @Query("SELECT m FROM Message m " + "WHERE m.sender = :sender AND m.receiver = :receiver " + "AND m.readAt IS NULL "
            + "AND m.isDeleted = false")
    ...
    @Query("SELECT m FROM Message m " + "WHERE m.sender = :sender AND m.receiver = :receiver "
            + "AND m.isDeleted = false")
    List<Message> findBySenderAndReceiverAndDeletedFalse(@Param("sender") User sender,
