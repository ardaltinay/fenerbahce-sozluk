...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        // Kullanıcılar
        User admin = userRepository.save(User.builder().username("admin").email("admin@fenerbahcesozluk.net")
                .password(passwordEncoder.encode("Admin123!")).role(Role.ADMIN).build());

        User user1 = userRepository.save(User.builder().username("sari_kanarya").email("sari@fenerbahce.com")
                .password(passwordEncoder.encode("Test123!")).role(Role.USER).build());

        User user2 = userRepository.save(User.builder().username("lacivert_aslan").email("lacivert@fenerbahce.com")
                .password(passwordEncoder.encode("Test123!")).role(Role.USER).build());

        User user3 = userRepository.save(User.builder().username("kadikoy_ruhu").email("kadikoy@fenerbahce.com")
                .password(passwordEncoder.encode("Test123!")).role(Role.USER).build());

        log.info("4 kullanıcı oluşturuldu");

        // Başlıklar
        Topic topic1 = topicRepository
                .save(Topic.builder().title("fenerbahçe").author(admin).entryCount(3).viewCount(1250L).build());

        Topic topic2 = topicRepository
                .save(Topic.builder().title("jose mourinho").author(user1).entryCount(2).viewCount(856L).build());

        Topic topic3 = topicRepository
                .save(Topic.builder().title("alex de souza").author(user2).entryCount(2).viewCount(2340L).build());

        Topic topic4 = topicRepository.save(
                Topic.builder().title("fenerbahçe basketbol").author(user1).entryCount(1).viewCount(432L).build());

        Topic topic5 = topicRepository
                .save(Topic.builder().title("kadıköy").author(user3).entryCount(2).viewCount(678L).build());

        log.info("5 başlık oluşturuldu");

        // Entry'ler
        // fenerbahçe başlığı
        entryRepository.save(Entry.builder().content(
                ...
                .topic(topic1).author(user1).likeCount(342).build());
        ...
        entryRepository.save(Entry.builder()
                ...
                .topic(topic1).author(user2).likeCount(234).build());

        entryRepository.save(Entry.builder()
                .content("benim için sadece bir kulüp değil, bir yaşam biçimi. her maç günü ayrı bir heyecan.")
                .topic(topic1).author(user3).likeCount(189).build());
